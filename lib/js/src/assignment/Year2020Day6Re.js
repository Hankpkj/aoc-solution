...
var input = Input.Input.getInput(/* Double */1, 6);
...
function getUnion(arr) {
  return Belt_SetString.size(Belt_Array.reduce(arr, emptyAlpha, Belt_SetString.union));
}
...
var l = Belt_Array.map(input, parsePhrase);

var union = Belt_Array.reduce(Belt_Array.map(l, getUnion), 0, sum);

console.log(union);

var intersect = Belt_Array.reduce(Belt_Array.map(l, getIntersect), 0, sum);
