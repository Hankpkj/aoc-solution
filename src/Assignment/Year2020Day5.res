...
let findMid = (r, ir, c) => Belt.List.reduce(r, ir, (acc, cur) => findRange(acc, cur === c)).mid
...
let sorted = ids->Belt.SortArray.stableSortBy((a, z) => a - z)->Belt.List.fromArray

let slidingWindow =
  sorted->Belt.List.tail->Belt.Option.mapWithDefault(list{}, tails => Belt.List.zip(sorted, tails))

let keeping =
  slidingWindow
  ->Belt.List.keep(((a, b)) => b - a !== 1)
