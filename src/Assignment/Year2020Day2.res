...
let split = string =>
  string->Js.String2.splitByRe(%re("/[ :\s | \- | \s ]/g"))->Belt.Array.keepMap(id)
...
let judgeX = policy => {
  let cnt =
    ...
    ->Js.String2.split("")
    ->Belt.List.fromArray
    ->Belt.List.keep(x => x === policy.target)
    ->Belt.List.size
...
let judgeY = policy => {
  ...
  let first =
    sourceToArr->Belt.Array.get(low - 1)->Belt_Option.mapWithDefault("Not Found", x => x) === target
  let second =
    sourceToArr->Belt.Array.get(high - 1)->Belt_Option.mapWithDefault("Not Found", x => x) ===
      target
  first !== second
}

// example 1 answer
li->Belt.List.keepMap(parsing)->Belt.List.keep(judgeX)->Belt.List.size->Js.log

// example 2 answer
li->Belt.List.keepMap(parsing)->Belt.List.keep(judgeY)->Belt.List.size->Js.log
